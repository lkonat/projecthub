// Library button: create a new GitHub repository for the project via the
// shared create-repo tool, then prompt for name/visibility/description first.
// Type-agnostic — reference it from a type's `buttons: [...]`. Auth comes from
// the GITHUB_TOKEN environment variable (see server/.env.example).
//
// On success the created repo's URLs are stashed in the project's free-form
// `meta` so a follow-up Clone can find them — and so this button disables
// itself to prevent accidental duplicates.

import gitCreateRepoTool from '../git/create-repo.js';
